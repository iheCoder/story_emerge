...
顶部“书架”会扫描数据根目录中拥有有效 `HEAD`、`project.json` 和 `story.json` 的小说。服务重启后，Agent 生成的书名、篇幅、章节进度和正文仍可恢复；点击作品会回到当前浏览器上次阅读的章节，未完成作品可以继续生成。
...
- `serve`：启动本地 Web 入口，提供持久化书架、三章试读、沉浸阅读和后续生成。
