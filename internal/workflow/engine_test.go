...
func TestJSONRepairRequestIncludesSchemaTypeFailure(t *testing.T) {
	...
	request := llm.Request{Stage: "chapter_004_reader", Role: llm.RoleReader, ReasoningEffort: "low"}
	...
	if repaired.Role != llm.RoleReader {
...
func TestMalformedOutputUsesChapterDiagnosticPrefix(t *testing.T) {
	// 场景：第二章 Reader 返回了错误字段类型。
	// 预期：诊断文件归入第 2 章前缀，而不是混进初始化阶段的 000 现场。
	if got := chapterNumberFromStage("chapter_002_reader_format_repair"); got != 2 {
